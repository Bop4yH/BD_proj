...
switch (sort_by.getSelectedIndex()){ // one case will execute in these cases 
    case 1 : {
    sql1 = "select NAME , TYPE ,BARCODE,SELLING_PRICE , EXPIRY  from drugs order by NAME";
    sort(1);  break;  //1- order by name from A - Z
    }
   case 2 : {
   sql2 = "select NAME , TYPE ,BARCODE,SELLING_PRICE , EXPIRY  from drugs order by TYPE";
   sort(2);break;  // 2- order by type from A - Z 
    }
    case 3 : {
   sql3 = "select NAME , TYPE ,BARCODE,SELLING_PRICE , EXPIRY  from drugs order by EXPIRY DESC";
...
private void drug_list(){ // this method will be executed when you click on drug_list
    String sql = "select NAME , TYPE ,BARCODE,SELLING_PRICE , EXPIRY  from drugs"; // get data from database
    try{
